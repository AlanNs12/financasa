import { cn } from '@/lib/utils'

interface SkeletonProps {
  variant?: 'card' | 'list' | 'chart' | 'text'
  className?: string
}

export function LoadingSkeleton({ variant = 'text', className }: SkeletonProps) {
  if (variant === 'card') {
    return (
      <div className={cn('bg-card rounded-2xl border border-border p-6 animate-pulse', className)}>
        <div className="h-4 bg-secondary rounded w-1/3 mb-4" />
        <div className="h-8 bg-secondary rounded w-1/2 mb-2" />
        <div className="h-3 bg-secondary rounded w-2/3" />
      </div>
    )
  }

  if (variant === 'list') {
    return (
      <div className={cn('space-y-3 animate-pulse', className)}>
        {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="flex items-center gap-4 p-3">
              <div className="w-10 h-10 rounded-xl bg-secondary shrink-0" />
              <div className="flex-1">
                <div className="h-4 bg-secondary rounded w-1/3 mb-1" />
                <div className="h-3 bg-secondary rounded w-1/4" />
              </div>
              <div className="h-4 bg-secondary rounded w-20" />
          </div>
        ))}
      </div>
    )
  }

  if (variant === 'chart') {
    return (
      <div className={cn('bg-card rounded-2xl border border-border p-6 animate-pulse', className)}>
        <div className="h-4 bg-secondary rounded w-1/4 mb-6" />
        <div className="h-48 bg-secondary rounded" />
      </div>
    )
  }

  return <div className={cn('h-4 bg-secondary rounded animate-pulse', className)} />
}

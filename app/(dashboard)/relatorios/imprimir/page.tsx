  ...
  return (
    <div className="max-w-3xl mx-auto p-8 print:p-0">
      ...
      <div className="no-print mb-6 flex items-center gap-3">
        <h1 className="text-lg font-bold text-gray-900">Relatório mensal</h1>
        <PrintButton />
      </div>

      <div className="space-y-6">
        <div className="border-b border-gray-200 pb-4">
          <h1 className="text-2xl font-bold text-gray-900">Financasa</h1>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="border border-gray-200 rounded-lg p-4">
            <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Receita</p>
        ...
        <div>
          ...
          {expensesByCategory.length === 0 ? (
            <p className="text-sm text-gray-400">Nenhum gasto no período.</p>
          ) : (
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b border-gray-200">
                  <th className="text-left py-2 pr-4 font-medium text-gray-700">Categoria</th>
                  <th className="text-right py-2 px-4 font-medium text-gray-700">Total</th>
                  <th className="text-right py-2 pl-4 font-medium text-gray-700">%</th>
                </tr>
              </thead>
              <tbody>
                {expensesByCategory.map((cat) => {
                  ...
                  return (
                    <tr key={cat.categoryId} className="border-b border-gray-100">
                      ...
                      <td className="py-2 px-4 text-right tabular-nums text-gray-900">
                        {formatCurrency(cat.total)}
                      </td>
                      <td className="py-2 pl-4 text-right tabular-nums text-gray-500">
                        {Math.round(pct)}%
                      </td>
                    </tr>
                  )
                ...
                <tr className="border-t-2 border-gray-300">
                  ...
                  <td className="py-2 px-4 text-right tabular-nums font-bold text-gray-900">
                    {formatCurrency(expenses)}
                  </td>
                  <td className="py-2 pl-4 text-right text-gray-500">100%</td>
                </tr>
              </tbody>
            </table>
          )}
        ...
        <div>
          ...
          {bills.length === 0 ? (
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b border-gray-200">
                  <th className="text-left py-2 pr-4 font-medium text-gray-700">Conta</th>
                  <th className="text-center py-2 px-4 font-medium text-gray-700">Vencimento</th>
                ...
                {bills.map((bill) => {
                  ...
                  return (
                    <tr key={bill.id} className="border-b border-gray-100">
                      <td className="py-2 pr-4 text-gray-900">{bill.name}</td>
                      ...
                      <td className="py-2 pl-4 text-center text-gray-600">
                        {statusLabel}
                      </td>
                    </tr>
                  )
